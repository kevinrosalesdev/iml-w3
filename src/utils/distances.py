import numpy as np


def euclidean_distances(X, Y, weights=None):
    if weights is None:
        weights = np.ones(X.shape[1])

    return [[np.sqrt(np.sum(np.multiply(weights, np.power(np.subtract(sample, other_sample), 2))))
             for other_sample in Y]
            for sample in X]


def manhattan_distances(X, Y, weights=None):
    if weights is None:
        weights = np.ones(X.shape[1])
        
    return [[np.multiply(weights, np.abs(np.subtract(x, y))).sum() for y in X] for x in Y]


def chebychev_distances(X, Y, weights=None):
    if weights is None:
        weights = np.ones(X.shape[1])

    return [[max(np.multiply(weights, np.abs(np.subtract(x, y)))) for y in X] for x in Y]
